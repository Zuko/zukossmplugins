...
//If set to true, your local currency will be displayed (in addition to EU prices)
var showYourLocalCurrency = true;
//Please set your local currency
var yourLocalCurrency = "PLN";
...
if (urlGamePattern.test(document.documentURI) || 
  ...
  if (showYourLocalCurrency) {
    ...
    loacalscript.setAttribute("src", 
        "http://javascriptexchangerate.appspot.com/?from=EUR&to="+yourLocalCurrency,true);
...
//    "  else if (currency == 'USD') {usdConverted = USDtoUSD(usdPrice);}\n" +
  ...
  for (i=0; i<pricenodes.length; i++) {
    ...
    if (showYourLocalCurrency) {
      try {
        priceHtml[5] = pricepattern6.exec(eu2Http.responseText)[1];
        price[5] = parseFloat(localvaluepattern.exec(priceHtml[5])[1]);
      }
      catch(err) {
        //Prevent search from looping around and starting at the beginning
        if (err.message.search("responseText\\) is null") != -1) {
          eu2Http = null; priceHtml[5] = "N/A";
        }
        if (!priceHtml[5] || priceHtml[5].length == 0)
          priceHtml[5] = "N/A";
        price[5] = null;
      }
      pricenodes[i].innerHTML += "<br>PL: " + priceHtml[5] 
          + " <span id='aud" + i + "'></span>"
      
      //createGetDifferenceScript("aud" + i, "USD", price[0], price[5]);
    }
  }
