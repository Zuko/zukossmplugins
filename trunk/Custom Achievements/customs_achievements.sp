...
new Handle:g_FLAGADMIN		= INVALID_HANDLE;
/*---------------------------------------------------------------------------------
...
new Handle:g_LIST_ACHIEVEMENTS_RESETATROUND		= INVALID_HANDLE;
//---Block A 
...
new g_ACHIEVEMENTSRESETDEATH	= 0;
new g_ACHIEVEMENTSRESETROUND	= 0;
new g_MAXOTHER		 	        = 0;
new g_USERS_ACH 		        = 0;
...
new g_BLOCKS_SAPPED		        = 0;      // Spy  Count
new g_BLOCKS_CALLMEDIC	        = 0;      // Call medic count
new g_BLOCKS_DEPLOYBANNER       = 0;      // Soldier banner count
new g_BLOCKS_ROCKETJUMP		    = 0;      // Soldier jump count
new g_BLOCKS_STICKYJUMP		    = 0;      // Demoman jump count
...
#include "mwach/soldier.inc"
#include "mwach/demoman.inc"
#include "mwach/hats.inc"
#include "mwach/menu.inc"
#include "mwach/resettimer.inc"
#include "mwach/admin.inc"
/*---------------------------------------------------------------------------------
I N F O  P L U G I N
---------------------------------------------------------------------------------*/
...
{    
	...
	// Create cvars
	CreateConVar("ach_version", PLUGIN_VERSION, "Current version of this plugin", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_UNLOGGED|FCVAR_DONTRECORD|FCVAR_REPLICATED|FCVAR_NOTIFY);
	g_CVDB 			= CreateConVar("ach_db", "achievement", "MySQL Database to use");
	g_CVTABLEPREFIX = CreateConVar("ach_prefix", "achmw_", "MySQL Database Table Prefix");
	g_CVURL 		= CreateConVar("ach_url", "http://mechaware.net/tf2/achievements/index.php", "Url to php file showing achievements",FCVAR_NOTIFY);
	RegAdminCmd		("ach_refresh", RequestRefresh, ADMFLAG_SLAY);
	g_CVINFO 	    = CreateConVar("ach_info", "0", "Show Achievement Info to Player on Unlocked? (0 = none, 1 = all, 2 = Player)",FCVAR_NOTIFY);
	g_CVMENU 	    = CreateConVar("ach_menu", "1", "Show Sourcemod menu to Player? (0 = no, 1 = yes)",FCVAR_NOTIFY);
	g_CVANNONCE	    = CreateConVar("ach_announce", "1", "Announce the plugin and possibility (0 = no, 1 = yes)",FCVAR_NOTIFY);
	g_CVCHATMOTD    = CreateConVar("ach_chatmotd", "/show_ach", "String for chat to show the motd page with player informations. (don't forget the /)",FCVAR_NOTIFY);
	g_CVCHATMENU    = CreateConVar("ach_chatmenu", "/ach", "String for chat to show the sourcemod menu with player informations. (don't forget the /)",FCVAR_NOTIFY);
	g_FLAGADMIN		= CreateConVar("ach_adminflag", "a", "Flags for identify an admin.",FCVAR_NOTIFY);
	AutoExecConfig(true, "customach_cfg");
	...
	RegConsoleCmd	("say", admin_part);
	RegConsoleCmd   ("say_team", motd_say);
	RegConsoleCmd   ("say_team", menu_say);
	RegConsoleCmd   ("say_team", blocks_say);
	RegConsoleCmd	("say_team", admin_part);
	
	HookEvent("teamplay_round_start", eventGameStart);
	HookEvent("teamplay_restart_round", eventGameStart);
	
	HookEvent       ("player_sapped_object", blocks_spy_sapped);
	HookEvent       ("deploy_buff_banner", blocks_deployBanner);
	HookEvent       ("rocket_jump", blocks_rocketjump);
	HookEvent       ("sticky_jump", blocks_stickyjump);
	...
	HookConVarChange(g_FLAGADMIN,         		UpdateCvar);
}
...
public CloseHandlePacks() 
{
	...
	MustCloseHandle (g_LIST_ACHIEVEMENTS_RESETATROUND);
	...
	MustCloseHandle (g_LIST_BLOCKS_STUN_STEAM_ID);
	...
	MustCloseHandle (g_LIST_BLOCKS_SAPPED_ID);
	MustCloseHandle (g_LIST_BLOCKS_SAPPED_WINNER);
	MustCloseHandle (g_LIST_BLOCKS_SAPPED_OBJECT);
	MustCloseHandle (g_LIST_BLOCKS_SAPPED_STEAM_ID);
	MustCloseHandle (g_LIST_BLOCKS_CALLMEDIC_ID);
	MustCloseHandle (g_LIST_BLOCKS_DEPLOYBANNER_ID);
	MustCloseHandle (g_LIST_BLOCKS_ROCKETJUMP_ID);
	MustCloseHandle (g_LIST_BLOCKS_STICKYJUMP_ID);
}
/*---------------------------------------------------------------------------------
MustCloseHandle
...
public CreateHandlePacks() 
{
	...
	g_LIST_ACHIEVEMENTS_RESETATROUND = CreateArray();
	...
	g_LIST_BLOCKS_UBERCHARGE_WEAPONS_UBER= CreateArray(arraySizeString);
	g_LIST_BLOCKS_UBERCHARGE_STEAM_ID= CreateArray(arraySizeString);
	
	g_LIST_BLOCKS_STUNNER_ID		= CreateArray();
	g_LIST_BLOCKS_STUNNER_WINNER	= CreateArray();
	g_LIST_BLOCKS_STUN_STEAM_ID 	= CreateArray(arraySizeString);
	
	g_LIST_BLOCKS_BONK_ID			= CreateArray();
	
	g_LIST_BLOCKS_JARATE_ID			= CreateArray();
	g_LIST_BLOCKS_JARATE_WINNER		= CreateArray();  
	g_LIST_BLOCKS_JARATE_STEAM_ID	= CreateArray(arraySizeString);
	
	g_LIST_BLOCKS_RAZORBACK_ID		= CreateArray();
	g_LIST_BLOCKS_RAZORBACK_WINNER	= CreateArray();
	
	g_LIST_BLOCKS_SAPPED_ID			= CreateArray();
	g_LIST_BLOCKS_SAPPED_WINNER		= CreateArray();
	g_LIST_BLOCKS_SAPPED_OBJECT		= CreateArray();
	g_LIST_BLOCKS_SAPPED_STEAM_ID	= CreateArray(arraySizeString);
	
	g_LIST_BLOCKS_CALLMEDIC_ID		= CreateArray();
	g_LIST_BLOCKS_DEPLOYBANNER_ID	= CreateArray();
	g_LIST_BLOCKS_ROCKETJUMP_ID		= CreateArray();
	g_LIST_BLOCKS_STICKYJUMP_ID		= CreateArray();
}
/*---------------------------------------------------------------------------------
ResetCache
...
RC_AchievementReset() {
	for(new i = 0; i < g_MAXACHIEVEMENTS; i++) 
	{
		if(GetArrayCell(g_LIST_ACHIEVEMENTS_RESETDEATH,i)==1)
		{
			...
			g_ACHIEVEMENTSRESETDEATH = g_ACHIEVEMENTSRESETDEATH + 1;
		}
		else
		{
			if(GetArrayCell(g_LIST_ACHIEVEMENTS_RESETDEATH,i)==2)
			{
				PushArrayCell(g_LIST_ACHIEVEMENTS_RESETATROUND, i);
				g_ACHIEVEMENTSRESETROUND = g_ACHIEVEMENTSRESETROUND + 1;
			}
		}
	}
}
...
{
	new strAchid = 0;
	for(new i = 0; i < g_ACHIEVEMENTSRESETDEATH; i++) 
	{
...
{
	new strAchid = 0;
	for(new i = 0; i < g_ACHIEVEMENTSRESETROUND; i++) 
	{
		strAchid = GetArrayCell(g_LIST_ACHIEVEMENTS_RESETATROUND,i);
		ResetFunction(strAchid,0);
	}
}
...
/*---------------------------------------------------------------------------------
Add + X on achievement statut
---------------------------------------------------------------------------------*/
AddToAchievement(any:argFocusclient, any:argId, any:data=1)
{
	...
	GetConVarString(g_CVTABLEPREFIX, strTablePrefix, sizeof(strTablePrefix));
	Format(strQuery, sizeof(strQuery), "INSERT INTO `%susers_ach` (`id`, `steamid`, `achid`, `status`, `complete`) VALUES ('%s', '%s', '%d', '1', '0') ON DUPLICATE KEY UPDATE `status` = `status` + %u", strTablePrefix, strId, strSteamId, argId,data);
...
public VerifyIfCompleteAch_AfterQuery(Handle:owner, Handle:strHQuery, const String:error[], any:argFocusclient)
{
	...
	for(new i = 0; i < g_MAXACHIEVEMENTS; i++) {
		...
		if (strAchid == argId) 
		{
			...
			InOneRound 		= GetArrayCell(g_LIST_ACHIEVEMENTS_RESETDEATH, i);
		}
	}
	...
	{
		VerifyofProgression(argFocusclient,strA_name,strA_status,strP_status,InOneRound);
	}
}


VerifyofProgression(argiUser, const String:argsAchievementName[], argAmount, argInterval,InOneRound)
{
	new strNewinterval;
	if(g_CONFIGPROGRESSION[argiUser] != 0)
	{
		...
		if(InOneRound==2)
		{
			...
			Format(strInOneLife, sizeof(strInOneLife),"%T", "In one round", LANG_SERVER);
			if(argAmount != 1)
				Format(strMessage, sizeof(strMessage), "\x05%s\x01, %s \x03(%i/%i - %s)", argsAchievementName, strMessageFile,strNewinterval,argAmount,strInOneLife);
			else
				Format(strMessage, sizeof(strMessage), "\x05%s\x01, %s \x03(%s)", argsAchievementName, strMessageFile,strInOneLife);
		}
		else
		{
...
stock SayText2One( argClient_index , argAuthor_index , const String:argMessage[] ) {
	if(IsValidClient(argClient_index) && IsValidClient(argAuthor_index))
	{
...
public eventGameStart(Handle:argEvent, const String:name[], bool:noBroadcast) 
{
	ResetAllAtRound();
}
