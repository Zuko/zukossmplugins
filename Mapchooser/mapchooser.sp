...
new Handle:g_WarningHintMsgTimer = INVALID_HANDLE;
...
public OnPluginStart()
{
	...
	OnPluginStart_WarningTime();//added
...
//added by Zuko
	...
	decl String:ws[PLATFORM_MAX_PATH];
	Format(ws,PLATFORM_MAX_PATH,"sound/%s",g_WarningSound);
//<-

	AddFileToDownloadsTable(ss);//added
	AddFileToDownloadsTable(es);//added
	AddFileToDownloadsTable(ws);//added

	PrecacheSound(g_StartSound, true);//added
	PrecacheSound(g_EndSound, true);//added
	PrecacheSound(g_WarningSound, true);//added
}
...
SetupWarningTimer()
{
	...
	if (GetMapTimeLeft(time) && time > 0)
	{
		...
		else
		{
			...
			g_WarningTimer = CreateTimer(float(time - startTime), Timer_WarningTimer, _, TIMER_FLAG_NO_MAPCHANGE);
		}

	}
}

public Action:Timer_WarningTimer(Handle:timer)
{
	g_WarningTimeStart = GetTime();
	g_WarningHintMsgTimer = CreateTimer(0.95, WarningHintMsg, _, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
...
public Action:WarningHintMsg(Handle:timer)
{
	...
	{
		KillTimer(g_WarningHintMsgTimer);
	}
}
