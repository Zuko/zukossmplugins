...
stock ProcessTemplateString(String:dest[], destlen, const String:source[]) {
	...
	new String:modOn[32], String:modOff[32], String:modYes[32],
	    String:modNo[32];
		
	Format(modOn, sizeof(modOn), "%T", "cOn" , LANG_SERVER);
	Format(modOff, sizeof(modOff), "%T", "cOff", LANG_SERVER);
	Format(modYes, sizeof(modYes), "%T", "cYes", LANG_SERVER);
	Format(modNo, sizeof(modNo), "%T", "cNo", LANG_SERVER);
			...
			if (cvh != INVALID_HANDLE) {
				...
				for (new j = 0; j < modcount; ++j) {
					if (strcmp(modifiers[j], "onoff") == 0) {
						if (strcmp(val, "0") == 0) strcopy(val, 4, modOff);
						else strcopy(val, 3, modOn);
					} else if (strcmp(modifiers[j], "yesno") == 0) {
						if (strcmp(val, "0") == 0) strcopy(val, 4, modNo);
						else strcopy(val, 3, modYes);
			...
			switch(ptypes[i]) {
				...
				case CVoteParamType_OnOff: {
					if (strcmp(vparams[i], "1") == 0)
						strcopy(replace, sizeof(replace), modOn);
					else
						strcopy(replace, sizeof(replace), modOff);
				}
				case CVoteParamType_YesNo: {
					if (strcmp(vparams[i], "1") == 0)
						strcopy(replace, sizeof(replace), modYes);
					else
						strcopy(replace, sizeof(replace), modNo);
				}
				default: {
...
Handle:CreateStockMenu(StockMenuType:menutype, MenuHandler:menuhandler, client, const String:mapcycle[] = "sm_cvote") {
	...
	switch(menutype) {
		...
		case StockMenuType_OnOff: {
			AddMenuItem(menu, "1", modOn, ITEMDRAW_DEFAULT);
			AddMenuItem(menu, "0", modOff, ITEMDRAW_DEFAULT);
		}
		case StockMenuType_YesNo: {
			AddMenuItem(menu, "1", modYes, ITEMDRAW_DEFAULT);
			AddMenuItem(menu, "0", modNo, ITEMDRAW_DEFAULT);
		}
	}
	return menu;
}
